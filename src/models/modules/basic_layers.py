...
class PatchEmbedding2D(nn.Module):
    def __init__(self, tensor_hw=(224, 224), patch_size=(16, 16), stride=(16, 16), in_channels=3, embed_dim=768, norm_layer=nn.Identity):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.flatten(2).transpose(1, 2)  # [N, embed_dim, grid_size[0], grid_size[1]] -> [N, num_patches, embed_dim]
        x = self.norm(x)
        return x


class PatchEmbedding3D(nn.Module):
    def __init__(self, tensor_dhw=(32, 224, 224), patch_size=(2, 16, 16), stride=(2, 16, 16), in_channels=3, embed_dim=768, norm_layer=nn.Identity):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        ...
        x = x.flatten(2).transpose(1, 2)  # [N, embed_dim, grid_size[0], grid_size[1], grid_size[2]] -> [N, num_patches, embed_dim]
        x = self.norm(x)
        return x